...
public class BlockDetector extends BlockContainer implements IDismantleable {
  ...
  public void registerBlockIcons(IIconRegister iconRegister) {
    ...
    this.backPowered = iconRegister.registerIcon("multimodplus:detector_back_powered");
  ...
  public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6,
    ...
    if (world.isRemote) {
      ...
      player.addChatMessage(
              new ChatComponentText("face : "
                      + ((TileEntityDetector) world.getTileEntity(x, y, z)).getFacing()));

      player.addChatMessage(
              new ChatComponentText("connected : "
   ...
   * Note: for mod blocks, this is up to the block and modder to decide.
   * It is not mandated that it be a rotation around the
  ...
  public ArrayList<ItemStack> dismantleBlock(EntityPlayer player, World world, int x,
                                             int y, int z, boolean returnDrops) {
    if (world.isRemote) {
      return null;
    }
